...
st.sidebar.markdown("Mortgage Loan Simulator")

with st.sidebar:
    home_value = st.number_input(
        "home value($): ", min_value=0.0, value=350000.0, format="%f"
    )

    interest_rate = st.number_input(
        "interest rate(%): ", min_value=0.0, format="%f"
    )

    down_payment_percent = st.number_input(
        "down payment (in %): ", min_value=0.1, value=4.0, format="%f"
    )

    payment_years = st.number_input(
        "payment period (years): ", min_value=3, format="%d"
    )
...
#     home_value = st.number_input(
#         "Enter your home value($): ", min_value=0.0, format="%f"
#     )

#     st.subheader("Loan Interest Rate")
#     interest_rate = st.number_input(
#         "Enter your home loan interest rate(%): ", min_value=0.0, format="%f"
#     )

# with col2:
#     st.subheader("Down Payment Percent")
#     down_payment_percent = st.number_input(
#         "Enter your down payment percent(%): ", min_value=0.0, format="%f"
#     )

#     st.subheader("Target Payment Period (Years)")
#     payment_years = st.number_input(
#         "Enter your target payment period (years): ", min_value=3, format="%d"
#     )
...
fig.add_trace(
    go.Scatter(x=month_num, y=principal_pay_arr, name="Principal Payment"), row=2, col=1
)

fig.append_trace(
    go.Scatter(x=month_num, y=interest_pay_arr, name="Interest Payment"), row=2, col=1
)

fig.update_layout(
    title="Mortgage Installment Payment Over Months",
    xaxis_title="Month",
    yaxis_title="Amount($)",
    height=800,
    width=1200,
    legend=dict(orientation="h", yanchor="top", y=0.47, xanchor="left", x=0.01),
)
...
fig = go.Figure()
fig.add_trace(
    go.Scatter(x=month_num, y=cumulative_home_equity, name="Cumulative Home Equity")
)

fig.add_trace(
    go.Scatter(x=month_num, y=cumulative_interest_paid, name="Cumulative Interest Paid")
)

fig.update_layout(
    title="Cumulative Home Equity Over Time",
    xaxis_title="Month",
    yaxis_title="Amount($)",
    height=500,
    width=1200,
...
cumulative_percent_owned = (down_payment_percent / 100) + (
    cumulative_home_equity / home_value
)
forecast_home_equity = cumulative_percent_owned * forecast_home_value

fig = go.Figure()
fig.add_trace(
    go.Scatter(x=month_num, y=forecast_home_value, name="Forecast Home Value")
)

fig.add_trace(
    go.Scatter(x=month_num, y=forecast_home_equity, name="Forecast Home Equity Owned")
)

fig.add_trace(
    go.Scatter(x=month_num, y=principal_remaining, name="Remaining Principal")
)

fig.update_layout(
    title="Forecast Home Value Vs Forecast Home Equity Over Time",
    xaxis_title="Month",
    yaxis_title="Amount($)",
    height=500,
    width=1200,
    legend=dict(orientation="h", yanchor="top", y=1.14, xanchor="left", x=0.01),
)
